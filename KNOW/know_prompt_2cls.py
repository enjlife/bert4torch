...
3.修改预测代码，不再输出label
4.添加 weight_loss 因为数据读取的不方便，转到文件 know_prompt_2cls_weight.py 修改了训练数据读取方式，修改了训练loss计算方式
"""
...
def load_dataset(path, p=None, mode='train'):
    ...
    for line in tqdm(fr):
        ...
        content, label, label2 = line[-1], line[1], line[2]
...
class KNOWTrainer(Trainer):
    ...
    def train(self, train_iter=None, dev_iter=None, test_iter=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for (token_ids, type_ids, labels, labels2) in tq_iter:
                token_ids, type_ids, labels, labels2 = token_ids.to(self.device), type_ids.to(self.device), labels.to(self.device), labels2.to(self.device)
...
if __name__ == '__main__':
    ...
    # 交叉验证
        ...
        if 'classifier1.weight' in weights:
            weights['classifier.weight'] = weights.pop('classifier1.weight')
            weights['classifier.bias'] = weights.pop('classifier1.bias')
        ...
        fw = open(args.data_dir + 'predict_dev.txt', 'w', encoding='utf8')
        lines = open(args.data_dir + 'test.txt', 'r', encoding='utf8').readlines()
        logger.info('res num: {}, score num: {}, lines num: {}'.format(len(res), len(score), len(lines)))
        id2label = {i: lab for i, lab in enumerate(args.labels)}
        for p, i, line in zip(score, res, lines):
