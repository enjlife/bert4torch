...
def preprocess(d, max_len=180):
    text, content, l_title, v_title, v_info, v_voice = d.get('TEXT', ''), d.get('CONTENT', ''), d.get('LURL_TITLE', ''), \
            d.get('VIDEO_TITLE', ''), eval(d.get('VIDEO_INFO', '{}')).get('display_name', ''), d.get('VIDEO_VOICE', '')
    if not text:
        text += content
...
class KNOWTrainer(Trainer):
    ...
    def train(self, train_iter=None, dev_iter=None, test_iter=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            tq_iter = tqdm(train_iter, desc="EP_%s:%d" % ('train', epoch + 1), ncols=40)  # bar_format="{l_bar}{r_bar}"
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        dev_data = load_dataset(args.data_dir + 'dev_r.txt', p=patterns[args.pattern], mode='dev')
    ...
    # 交叉验证
        ...
        cls_model = BertForNextSentencePrediction.from_pretrained(args.pretrained_path)
        ws = torch.load(args.save_path)
        ws['classifier.weight'] = ws['classifier1.weight']
        ws['classifier.bias'] = ws['classifier1.bias']
        cls_model.load_state_dict(ws)
        cls_model.to(device)
        cls_model.eval()
        ...
        for file in files:
            ...
            with torch.no_grad():
                for (token_ids, type_ids, _) in tqdm(test_it):
                    ...
                    logits, _ = cls_model(token_ids, type_ids)
                    logits = F.softmax(logits, dim=-1)
